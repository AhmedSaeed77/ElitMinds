...
@section('pageTitle') Update Explanation @endsection
@section('subheaderTitle') Update Explanation @endsection
    ...
    <div class="row" id="app-1" style="background-color: white; padding: 0 0;">
        <div class="col-md-12">
            ...
            <div class="form-horizontal" style="background: white; padding: 30px 15px;">
                ...
                <div class="form-group row">
                    <label class="col-2 col-form-label" for="title" >Title_ar :</label>
                    <div class="col-10">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-10">
                        <select class="form-control" id="exampleSelectd1"  v-model.number="course">
    ...
    <script>
        const app = new Vue({
            ...
            mounted() {
                ...
                this.contant = this.initEditor('contant', 280);
                this.contentEditor_ar = this.initEditor('contentEditor_ar', 280);
                this.loader();
            ...
            methods: {
                loader: function(){
                    ...
                    $.ajax ({
                        type: 'POST',
                        url: '{{ route('admin.show.FaqCorce', $post_id)}}',
                        success: function(res){
                            console.log("res")
                            while(!window.CKEDITOR);
                            KTApp.unblockPage();
                            app.title = res['title'];
                            app.title_ar = res['title_ar'];
                            app.contant.setData(res['contant']);
                            app.contentEditor_ar.setData(res['content_ar']);
                            app.course = res['cource_id'];
                        },
                        error: function(err){
                            KTApp.unblockPage();
                            console.log(err);
                ...
                update: async function(){
                    ...
                    await app.updateRequest().then(() => {
                        ...
                        swal({
                            ...
                            window.location.replace('{{route('admin.FaqCorce')}}');
                        });
                    });
                },
                updateRequest: async function(){
                    return $.ajax ({
                        type: 'POST',
                        url: '{{ route('admin.update.FaqCorce')}}',
                        data: {
                          
                            id: $post_id,
                             title: app.title,
                            title_ar: app.title_ar,
                            content: app.contant.getData(),
                            content_ar: app.contentEditor_ar.getData(),
