...
<head>
    ...
    <title>{{ env('APP_NAME') }}| Rest Password</title>
                    ...
                    <div class="w-100 w-lg-75 w-xxl-50">
                        <div>
                            <div class="mb-5">
                                ...
                                <h1 class="display-3 text-white">Are You Ready to learn ?</h1>
                            </div>
            ...
            <div class="col-12 col-lg-auto h-100 pb-4 px-4 pt-0 p-lg-0">
                <div class="sw-lg-70 min-h-100 bg-foreground d-flex justify-content-center align-items-center shadow-deep py-5 full-page-content-right-border">
                    <div class="sw-lg-50 px-5">
                        ...
                        <div class="mb-5">
                            <p class="h6">{{__('Public/auth.forget-password')}}</p>
                            <p class="h6">
                                {{__('Public/auth.reset-account-password')}}
                                or Continue to <a href="{{route('login')}}">Login</a>
                                .
                            </p>
                        </div>
                        @if (session('status'))
                        ...
                        @endif
                        <div>
                            <form id="loginForm" class="tooltip-end-bottom" action="{{ route('password.email') }}" method="POST" novalidate>
                                {{ csrf_field() }}
                                <div class="mb-3 filled form-group tooltip-end-top">
                                    <i data-cs-icon="email"></i>
                                    <input type="email" class="form-control" placeholder="Email" name="email" value="{{ old('email') }}" required autofocus/>
                                </div>
                                <button type="submit" class="btn btn-lg btn-primary">{{__('Public/auth.reset')}}</button>
                            </form>
                        </div>
                    </div>
                </div>
