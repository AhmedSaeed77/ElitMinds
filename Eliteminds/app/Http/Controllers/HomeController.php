...
class HomeController extends Controller
{
    ...
    public function __construct()
    {
        ...
        //          'store_termsOfSevice',
        //           'securityCheck',
        //            'eventPage', 'activateAccount', 'freeResourceDemoVideos', 'reviews', 'aboutUs', 'showPost', 'indexBlog', 'contactPage', 'getVimeoVideoHtml'
        // ]); //default auth --->> auth:web
    ...
    public function indexBlog(PostRepositoryInterface $postRepository)
    {
     ...
     public function indexBlogelement(PostRepositoryInterface $postRepository)
    {
        $posts = $postRepository->all([], request()->section_id);
        $arr = [];
        foreach($posts as $post)
    ...
    public function certificates(Locale $locale, PackageRepositoryInterface $packageRepository)
    {
        ...
        //         'topics'        => $courses_details->filter(function ($row) use ($course) {
        //             return $row->course_id == $course->id;
        //         })
        //     ]);
        // }
        $courses = Cache::remember('coursesIndexPage', 1440, function () {
            return \App\Course::where('private', '=', 0)->orderBy('updated_at', 'desc')
                ->get();
        });
        $country_code = Zone::getLocation()->country_code;
        $courses_details = DB::table('course_details')->whereIn('course_id', $courses->pluck(['id']))->get();
        foreach ($courses  as $co) {
            $packagescourse = $packageRepository->getPackagesByCourse($co->id, $country_code, $locale->locale);
            $houres = 0;
            $lessons_number = 0;
            $rate = 0;
            // dd($packagescourse);
            foreach ($packagescourse as $pack) {
                $rate = $rate + $pack->package->rate;
                $houres = $houres + $pack->package->total_time;
                $lessons_number = $lessons_number + $pack->lessons_number;
            }
            $co->houres = $houres;
            $co->lessons_number = $lessons_number;
            $co->rate = $rate;
        }
        //dd($courses_details);
        
        $usercourses = DB::table('packages')
                        ->leftJoin('user_packages', 'packages.id', '=', 'user_packages.package_id')
                        ->groupBy('packages.course_id')
                        ->select(
                                    DB::raw('count(DISTINCT user_packages.user_id) AS user_packages'),
                                    DB::raw('packages.course_id AS course_id')
                                )
                        ->get();
                        // dd($usercourses);
                        
        return view('course.all', compact('courses', 'courses_details','usercourses'));
    }

    public function showPost(Locale $locale, PostRepositoryInterface $postRepository, $slug)
    {
          $locale = new Locale;
        ...
        $postnew = Post::find($post->id);  
        event (new PackageCounter($postnew));
        return view('blog.singleV3')
            ->with('post', $post);
    }
    ...
    public function aboutUs()
    {
        ...
        return view('course.newaboutus')->with('userFeedBack', $userFeedBack);
    }
    ...
    public function user_board(Locale $locale)
    {
        ...
        foreach ($userPackages as $package) {
            $videosProgress = DB::table('videos')
                ...
                ->leftJoin(
                    DB::raw('(SELECT * FROM video_progresses WHERE package_id=' . $package->package_id . ' AND user_id=' . $thisUser->id . ' GROUP BY video_id) AS video_progresses'),
                    'videos.id',
                    '=',
                    'video_progresses.video_id'
                )
                ->select(
    ...
    public function package_view(Request $req, $id)
    {
        ...
        if (!$package) {
            ...
            if ($package->slug) {
                $url = route('public.package.view',  $package->slug);
                if (request()->coupon) {
                    $url .= '?coupon=' . request()->coupon;
                }
        ...
        if ($package) {
            ...
            return view("course.checkout")->with('i', $item)
                ->with('rating', $rating)
  ...
  $v_id = 624705745;
    ...
    public function NewIndex(
        ...
        return response()->json(compact('promotedCouponDetails', 'webSiteStatistics', 'courses', 'popular_courses', 'userFeedBack', 'faq', 'feedbackRating', 'posts'));
    }
    ...
    public function allCours(
        ...
        $country_code = Zone::getLocation()->country_code;
        $popular_courses = $packageRepository->getPopularPackages2($country_code, $locale->locale);
        return view('allcourse')->with('popular_courses', $popular_courses);
    }
    ...
    public function updateprofile(Request $request)
    {
        ...
        $country = \App\Country::where('id',$request->country)->first();
        ...
        $user->save();                
        //return FacadesRedirect::route('user.dashboard');
        return FacadesRedirect::back();
