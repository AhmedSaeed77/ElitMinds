...
class PremiumQuizController extends Controller
{
    ...
    public function QuizHistory_show($id){
        ...
        if($quiz){
             $exams = \App\Quiz::where('package_id', $quiz->packzge_id)->where('user_id', $quiz->user_id)->get();
            if ($exams) {
                foreach ($exams as $ex) {
                    if ($ex->topic_type == 'chapter') {
                        $ex->topic = \App\Chapters::find($ex->topic_id)->name;
                    } elseif ($ex->topic_type == 'process') {
                        $ex->topic = \App\Process_group::find($quiz->topic_id)->name;
                    } elseif ($ex->topic_type == 'exam') {
                        $ex->topic = \App\Exam::find($quiz->topic_id)->name;
                    } elseif ($ex->topic_type == 'mistake') {
                        if ($ex->topic_type == 1) {
                            $ex->topic = 'Chapter Mistakes';
                        } else if ($ex->topic_type == 2) {
                            $ex->topic = 'Process Group Mistakes';
                        } else if ($ex->topic_type == 3) {
                            $ex->topic = 'Exam Mistakes';
                        }
                    }
                    $ex->score = $quiz->score;
                }
...
//                    $join->on('wrong_drag_right_answers.question_id', '=', 'questions.id');
                ...
                ->groupBy('questions.id')
                ->get()->count();
            $rating = DB::table('ratings')->where('package_id', $userPackage->package_id)
            ->select(
            ...
            $rating_reviews = DB::table('ratings')->where('package_id', $userPackage->package_id)
...
//        dd(DB::getQueryLog());
    ...
    public function material_show($course_id)
    {
        ...
        if($userPackages)
        {
            foreach($userPackages as $i)
            {
                ...
                if($course_id == 0)
                {
                    $course_id = $package->course_id[0];
                }
                if($package->course_id == $course_id && ($package->contant_type == 'combined' || $package->contant_type == 'video' || $package->contant_type == 'question') )
                {
                    $has = 1;
                    break;
                }
        ...
        if(!$has){
         ...
         return view('matriales.index')->with('course_id', $course_id);
    }
     public function StudyMetrial()
    {
           $locale = new Locale();
        // check if user is subjected to this course
        $has = 0;
        ...
        if($userPackages
            ...
            foreach($userPackages as $key => $i)
            {
                $package = \App\Packages::find($i->package_id);
                if($course_id == 0 && $key == 0)
                {
    ...
    public function feedback_index(Locale $locale){
         $packages =  $this->UserPackage(null, Auth::user()->id);
        //  dd($packages);
        return view('user.newfeedback' , compact('packages'));
    }
    ...
    public function storeReview(Request $req){
        ...
        if($req->has('package_id')){
            ...
            if($rate){
             ...
             $new = new \App\Rating();
             $new->package_id = $req->input('package_id');
             $new->user_id =  Auth::user()->id;
             $new->review = $req->input('user_review');
             $new->save();
            }
        }else if($req->has('event_id')){
            $rate = \App\Rating::where('user_id', Auth::user()->id)->where('event_id', $req->input('event_id'))->get()->first();
