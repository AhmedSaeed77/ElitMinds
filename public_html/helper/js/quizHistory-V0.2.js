  ...
  computed: {
    ...
    currentSelectedAnswersCount: function () {
      if (this.isFillInTheBlank || this.isMultipleResponses) {
        return this.user_answers[this.getCurrentQuestion().id].answers.filter(
          function (row) {
            return row.selected;
          }
        ).length;
  ...
  methods: {
    ...
    renderDragCenterChoice: function (choice, target, sub_target) {
      ...
      if (sub_target) {
        ...
        if (this.language != 'en') {
          const answer_localized =
            choice[target].transcodes[sub_target + '_' + this.language];
        ...
        if (this.language != 'en') {
          const answer_localized =
            choice.transcodes[target + '_' + this.language];
          answer = answer_localized ? answer_localized : answer;
        }
      }
      return answer;
    ...
    prev: function () {
      /** validation */
      if (
        app.current_question_number - 1 <= 0 ||
      ...
      switch (this.answers_filter) {
        ...
        case '0':
          ...
          filteredAnswers = this.allQuestions.filter((row) => {
            return (
              !app.user_answers[row.id].user_answer_is_correct &&
    ...
    markExam: function () {
      /** Mark User Answers */
      for (const [question_id, user_answer] of Object.entries(
        app.user_answers
      )) {
      ...
      /** overall score */
      app.overallScore =
        (Object.values(app.user_answers).filter(
          (row) => row.user_answer_is_correct
        ).length /
          app.questions.length) *
        100;
      ...
      if (app.overallScore > 75) {
        this.ScoreMsg =
          ...
          quizHistoryAttr.translation.overallPerformance +
          '<b style="color:springgreen;">' +
          quizHistoryAttr.translation.passed +
          '</b></h4>You have passed the exam, congratulations.';
      } else {
        this.ScoreMsg =
          '<h4 style="font-weight:bold">' +
          quizHistoryAttr.translation.overallPerformance +
          ': <b style="color:#DC143C;">' +
          quizHistoryAttr.translation.failed +
          '</b></h4>You have failed your exam, you must improve to get certified.';
      }
      /** Result by process */
      ...
      html = '';
      for (const [chapter, user_answers_] of Object.entries(
        userAnswersByChapters
      )) {
        correctAnswersCount = user_answers_.filter(
          (row) => row.user_answer_is_correct
        ).length;
        ...
        html +=
          ...
          chapter +
          '</td><td>' +
          user_answers_.length +
          '</td><td>' +
          correctAnswersCount +
          '</td><td>' +
          Math.round(chapterScore) +
          '</td></tr>';
      }
      ...
      for (const [process_group, user_answers_] of Object.entries(
        userAnswersByProcess
      )) {
        for (const answer_ of user_answers_) {
          if (answer_.user_answer_is_correct) {
            if (this.current_question_flag) {
              html +=
                '<tr><td>' +
                answer_.question_number +
                '</td><td>' +
                (answer_.chapter ?? '--') +
                '</td><td><svg width="19" height="23" viewBox="0 0 19 23" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1.75781 14.5C1.75781 14.5 2.72641 13.5 5.63222 13.5C8.53803 13.5 10.4752 15.5 13.381 15.5C16.2868 15.5 17.2555 14.5 17.2555 14.5V2.5C17.2555 2.5 16.2868 3.5 13.381 3.5C10.4752 3.5 8.53803 1.5 5.63222 1.5C2.72641 1.5 1.75781 2.5 1.75781 2.5V21.5" stroke="#F58634" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></td><td><i style="color:green">1</i></td></tr>';
            } else {
              html +=
                '<tr><td>' +
                answer_.question_number +
            ...
            if (this.current_question_flag) {
              html +=
                ...
                '</td><td>' +
                (answer_.chapter ?? '--') +
                '</td><td><svg width="19" height="23" viewBox="0 0 19 23" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1.75781 14.5C1.75781 14.5 2.72641 13.5 5.63222 13.5C8.53803 13.5 10.4752 15.5 13.381 15.5C16.2868 15.5 17.2555 14.5 17.2555 14.5V2.5C17.2555 2.5 16.2868 3.5 13.381 3.5C10.4752 3.5 8.53803 1.5 5.63222 1.5C2.72641 1.5 1.75781 2.5 1.75781 2.5V21.5" stroke="#F58634" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></td><td><i style="color:red">0</td></tr>';
            } else {
              html +=
                '<tr><td>' +
                answer_.question_number +
                '</td><td>' +
                (answer_.chapter ?? '--') +
                '</td><td><svg width="19" height="23" viewBox="0 0 19 23" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1.75781 14.5C1.75781 14.5 2.72641 13.5 5.63222 13.5C8.53803 13.5 10.4752 15.5 13.381 15.5C16.2868 15.5 17.2555 14.5 17.2555 14.5V2.5C17.2555 2.5 16.2868 3.5 13.381 3.5C10.4752 3.5 8.53803 1.5 5.63222 1.5C2.72641 1.5 1.75781 2.5 1.75781 2.5V21.5" stroke="#BFC1C1" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></td><td><i style="color:red">0</td></tr>';
            }
          }
        }
      }
    ...
    push_question: function (CQN /* Current question Number # start from 1*/) {
      ...
      this.current_question_type_id =
        this.getCurrentQuestion().question_type_id;
      this.current_question_flag =
        this.user_answers[this.getCurrentQuestion().id].flag;
      this.current_question_correct_answers_required =
        this.getCurrentQuestion().correct_answers_required;
      this.current_question_status = this.user_answers[this.currentQuestionId]
        .user_answer_is_correct
        ? 'correct'
        : this.user_answers[this.currentQuestionId].skipped
        ? 'skipped'
        : 'wrong';

      if (
        this.isMultipleChoice ||
        this.isFillInTheBlank ||
        this.isMultipleResponses
      ) {
      ...
      for (i = 0; i < app.questions.length; i++) {
          ...
          if (res.answers[app.questions[i].id]) {
            if (app.questions[i].question_type_id == 1) {
              answer = {
                ...
                answer_id:
                  res.answers[app.questions[i].id][0].question_answer_id,
              };
            ...
            if (res.answers) {
              if (res.answers[app.questions[i].id]) {
                ...
                item = res.answers[app.questions[i].id].filter((row) => {
                  return (
                    row.question_answer_id == app.questions[i].answers[j].id
                  );
                });
                if (item.length) {
          ...
          for (j = 0; j < app.questions[i].drag_right.length; j++) {
            ...
            if (res.dragRightAnswers) {
              if (res.dragRightAnswers[app.questions[i].id]) {
                left_item = res.dragRightAnswers[app.questions[i].id].filter(
                  (row) => {
                    return (
            ...
            // right side setup
            app.user_answers[app.questions[i].id].right[
              app.questions[i].drag_right[j].id
            ] = {
        ...
        if (app.questions[i].drag_center) {
          for (j = 0; j < app.questions[i].drag_center.length; j++) {
            ...
            app.user_answers[app.questions[i].id].center[drag_center_j.id] = {
              ...drag_center_j,
              left: {
                selected: randFlag
                  ? userAnswer == drag_center_j.correct_sentence
                  : userAnswer == drag_center_j.wrong_sentence, // must be from history
                left_sentence: randFlag
                  ? drag_center_j.correct_sentence
                  : drag_center_j.wrong_sentence,
                transcodes: {
                  left_sentence: randFlag
                    ? drag_center_j.transcodes.correct_sentence
                    : drag_center_j.transcodes.wrong_sentence,
                },
              },
              right: {
                selected: randFlag
                  ? userAnswer == drag_center_j.wrong_sentence
                  : userAnswer == drag_center_j.correct_sentence, // must be from history
                right_sentence: randFlag
                  ? drag_center_j.wrong_sentence
                  : drag_center_j.correct_sentence,
                transcodes: {
                  right_sentence: randFlag
    ...
    getCurrentQuestionImage: function () {
      if (this.getCurrentQuestion().img)
        return (
          quizHistoryAttr.base_url +
          '/storage/questions/' +
          this.baseName(this.getCurrentQuestion().img)
        );
      else return null;
    },
    /** Title */
    getCurrentTitle: function (getLanguage = null) {
      if (!this.getCurrentQuestion()) return '';
