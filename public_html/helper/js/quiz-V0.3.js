var app = new Vue({
    ...
    methods: {
        getNotes: function () {
                  ...
                  return $.ajax({
                    type: "POST",
                    url: quizAttr.api.getNotes,
                    data: {
                      video_id: this.video_id,
                    },
                    success: function (ress) {
                     console.log(ress)
                     app.Notes = ress.notes;
                    },
                    error: function (res) {
        ...
        getRate: function () {
                  ...
                  return $.ajax({
                    type: "POST",
                    url: quizAttr.api.getRate,
                    data: {
                      package_id: this.package_id
                    },
                    success: function (ress) {
                      this.rates = ress.rate;
                    },
                    error: function (res) {
                      console.log("Error:", res);
                    },
        ...
        itemHighlighted: function (topic_type, topic_id) {
            return (topic_type == this.topic_type && topic_id == this.topic_id)
                ...
                || (topic_type == 'explanation' && topic_id == this.explanation_id) ? 'active' : '';
        },
        isItemActive: function(topic_type, topic_id){
            return (topic_type == this.topic_type && topic_id == this.topic_id)
                || (topic_type == 'video' && topic_id == this.video_id)
                || (topic_type == 'explanation' && topic_id == this.explanation_id);
        },
        feedMeBack: function () {
